...
#if defined(_WIN32)
#define KTTS_EXPORT __declspec(dllexport)
#else
#define KTTS_EXPORT __attribute__((visibility("default"))) __attribute__((used))
#endif
...
KTTS_EXPORT PhonemizerHandle phonemizer_create(const char* rules_path,
...
KTTS_EXPORT void phonemizer_destroy(PhonemizerHandle handle) {
    if (handle) {
...
// Returns NULL on failure
KTTS_EXPORT char* phonemizer_phonemize(PhonemizerHandle handle, const char* text) {
...
KTTS_EXPORT void phonemizer_free_string(char* str) {
    free(str);
}

// Get error message from a failed create
KTTS_EXPORT const char* phonemizer_get_error(PhonemizerHandle handle) {
